...
/* The use-case paths are derived from the same data the pages and the nav read,
 * rather than listed by hand, so a new use case cannot be added without also
 * appearing in the sitemap. No trailing slashes: Metadata strips them from the
 * canonical, and Next 308s /use-cases/ to /use-cases. */
...
async function buildHomeMarkdown() {
  ...
  const faqs = LandingPageData?.MainSiteFAQs || [];
...
async function buildPricingMarkdown() {
  const lines = [
    "# Lighthouse Pricing",
    "",
    "## Filecoin Plans (Monthly)",
    "",
    formatPlanSection(FilecoinMonthlyPricing, "per month"),
    "",
    "## Filecoin Plans (Annual)",
    "",
    formatPlanSection(FilecoinAnnualPricing, "per year"),
    "",
    "## Walrus Plans (Monthly)",
    "",
    formatPlanSection(WalrusMonthlyPricing, "per month"),
    "",
    "## Add-on Plans",
    "",
...
/* The use-case pages are the richest prose on the memory side of the site, and
 * /sitemap.md advertises a .md alternate for every canonical path, so they need
 * a renderer here or those URLs 404. Both builders read the same useCases data
 * the pages render, so the markdown cannot drift from the HTML. */
function buildUseCasesIndexMarkdown() {
  const lines = [
    "# Use cases",
    "",
    "Memory for trading agents, prediction markets, tokenised assets and physical AI.",
    "",
    "## Use cases",
    "",
  ];
  ...
  useCases.forEach((useCase) => {
    ...
    lines.push(`- ${markdownLink(safeText(useCase.name), path)}`);
    lines.push(`  - ${safeText(useCase.description)}`);
    if (useCase.planned) {
      lines.push("  - Status: reference integration, planned");
    }
  });
...
function buildUseCaseDetailMarkdown(rawSlug) {
  ...
  const lines = [
    `# ${safeText(useCase.name)}`,
    "",
    `${safeText(useCase.lead)} ${safeText(useCase.headline)}`,
    "",
    safeText(useCase.description),
    "",
  ];

  if (useCase.planned) {
    lines.push("> Reference integration, planned.", "");
  }

  if (useCase.records?.length) {
    lines.push("## What is recorded", "");
    useCase.records.forEach(([label, detail]) => {
      lines.push(`- **${safeText(label)}**: ${safeText(detail)}`);
    });
    lines.push("");
  }

  if (useCase.benefits?.length) {
    lines.push("## How it helps", "");
    useCase.benefits.forEach(([heading, body]) => {
      lines.push(`### ${safeText(heading)}`, "", safeText(body), "");
    });
  }

  if (useCase.steps?.length) {
    lines.push("## The loop", "");
    useCase.steps.forEach((step, index) => {
      lines.push(`${index + 1}. ${safeText(step)}`);
    });
    lines.push("");
  }

  if (useCase.note) {
    lines.push("## Scope", "", safeText(useCase.note), "");
  }
...
async function resolveMarkdownByPath(pathname) {
  ...
  // /storage renders the same tree as the homepage, so it serves the same
  // markdown. When the root becomes the memory page these diverge.
  if (pathname === "/storage") {
    return buildHomeMarkdown();
  }
  if (pathname === "/pricing") {
  ...
  if (pathname === "/use-cases") {
    return buildUseCasesIndexMarkdown();
  }

  if (pathname.startsWith("/use-cases/")) {
    return buildUseCaseDetailMarkdown(pathname.slice("/use-cases/".length));
  }
